...
def apply_action(body, actions, joints, powers):
	forces = [0. for i in range(len(joints))]
	for m in range(len(joints)):
		# limit = 1
		# ac = np.clip(actions[m], -limit, limit)
		# forces[m] = powers[m] * ac * 10

		power_global = 0.41
		power = powers[m]
		force_gain = 1
		force = float(force_gain * power * power_global * np.clip(actions[m], -1, +1))
		forces[m] = force
		p.setJointMotorControl2(body, joints[m], controlMode=p.TORQUE_CONTROL, force=force)
...
if __name__ == "__main__":
	...
	humanoid = bullet_client.loadMJCF("mjcf/humanoid_symmetric.xml")[0]

	bullet_client.createConstraint(humanoid, -1, -1, -1, p.JOINT_FIXED, [0, 0, 0], [0, 0, 0, 1], [0, 0, 2])

	bodies = [humanoid]
	parts, joints, ordered_joints, robot_body = addToScene(bullet_client, bodies)
	...
	motor_power += [75, 75, 75]
	motors = [joints[n] for n in motor_names]

	force_gain = 1
	global_power = 0.41

	params = [bullet_client.addUserDebugParameter(motor.joint_name, -1, +1) for motor in motors]

	while bullet_client.isConnected():

		actions = [bullet_client.readUserDebugParameter(param) for param in params]

		for i, m, power in zip(range(17), motors, motor_power):
			m.set_motor_torque(float(force_gain * power * global_power * np.clip(actions[i], -1, +1)))
